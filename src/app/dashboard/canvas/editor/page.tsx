...
function EditorInner() {
  ...
  const saveAnimationFrame = (frame = currentFrameRef.current, layerId = selectedAnimLayerIdRef.current) => {
    ...
    const layerLiveObjects = objects.filter((o:any) => o.__animLayerId === layerId);
  ...
  const waitForAnimationFrame = async (frame:number) => {
  ...
  const updateFillGradient = (g: GradValue|null) => {
    ...
    if (!g) { upd({ fill: selFill }); if (sel?.__threeD?.enabled) requestAnimationFrame(() => refreshThreeDObject(sel)); return; }
  ...
  const supported3DShape = (obj:any) => {
    if (!obj) return false;
  ...
  async function refreshThreeDObject(obj:any, patch?:Partial<Shape3DSettings>) {
  ...
  const updateCharSpacing = (v: number) => { setSelCharSpacing(v); upd({ charSpacing: v }); if (sel?.__threeD?.enabled) requestAnimationFrame(() => refreshThreeDObject(sel)); };
        ...
        <div className="w-56 bg-white border-l border-gray-200 flex flex-col flex-shrink-0 overflow-y-auto text-xs">
          ...
          {!pixelEditMode && (sel ? (
            <div className="p-3 flex flex-col gap-3 border-b border-gray-200">
              ...
              {sel.type !== "activeSelection" && sel.__motionPath && (() => {
      ...
      {animationMode && (
        <div className="h-52 bg-white border-t border-gray-200 flex-shrink-0 flex flex-col shadow-[0_-8px_24px_rgba(15,23,42,0.05)] z-30">
          <div className="h-10 px-3 border-b border-gray-100 flex items-center gap-2 flex-shrink-0">
            ...
            <button onClick={exportAnimationWebM} disabled={animationExporting}
