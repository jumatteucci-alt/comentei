...
# Insert layer easing updater before removeAnimationLayer.
...
# Add easing control near FPS in timeline toolbar.
...
# Make tweened cells visually distinct between keyframes.
old = '''                      const isKey = layer.frames[f] !== undefined;\n                      const active = currentFrame === f;'''
new = '''                      const isKey = layer.frames[f] !== undefined;\n                      const keys = Object.keys(layer.frames).map(Number).sort((a,b)=>a-b);\n                      const hasPrev = keys.some(k => k < f);\n                      const hasNext = keys.some(k => k > f);\n                      const isTween = !isKey && hasPrev && hasNext;\n                      const active = currentFrame === f;'''
assert old in s, 'timeline cell state marker missing'
s = s.replace(old, new, 1)

old = '''                          {isKey && <span className="text-indigo-600 text-[12px] leading-none">◆</span>}'''
new = '''                          {isKey && <span className="text-indigo-600 text-[12px] leading-none">◆</span>}\n                          {isTween && <span className="w-1 h-1 rounded-full bg-indigo-200" title="Frame interpolado" />}'''
assert old in s, 'timeline keyframe diamond marker missing'
s = s.replace(old, new, 1)
